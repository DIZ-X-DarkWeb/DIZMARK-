...
function renderRegisterPage(container) {
  container.innerHTML = '<div class="max-w-md mx-auto py-12 animate-slide-in"><div class="text-center mb-8"><img src="https://files.catbox.moe/pe25yl.png" alt="DIZMARK" class="h-24 w-24 mx-auto rounded-full mb-4"><h2 class="text-2xl font-bold gradient-text">Metode Pendaftaran</h2></div><div class="space-y-4"><button onclick="loginWithGoogle()" class="w-full glass-card p-4 rounded-xl flex items-center justify-center gap-3 hover:bg-blue-600/30 transition"><span class="text-2xl">G</span><span class="text-lg">Lanjutkan dengan Google</span></button><button onclick="loginWithGitHub()" class="w-full glass-card p-4 rounded-xl flex items-center justify-center gap-3 hover:bg-blue-600/30 transition"><span class="text-2xl">GH</span><span class="text-lg">Lanjutkan dengan GitHub</span></button><div class="text-center text-blue-300 mt-4">Sudah memiliki akun? <span onclick="navigateTo(\'login\')" class="text-blue-400 cursor-pointer hover:underline">Login</span></div></div></div>'
}

// LOGIN PAGE
function renderLoginPage(container) {
  container.innerHTML = '<div class="max-w-md mx-auto py-12 animate-slide-in"><div class="text-center mb-8"><img src="https://files.catbox.moe/pe25yl.png" alt="DIZMARK" class="h-24 w-24 mx-auto rounded-full mb-4"><h2 class="text-2xl font-bold gradient-text">Masuk ke Akun Anda</h2></div><div class="space-y-4"><input type="text" id="login-email" placeholder="Nama pengguna atau email Anda" class="input-diz w-full p-4 rounded-xl text-lg"><div class="relative"><input type="password" id="login-password" placeholder="Kata Sandi Anda" class="input-diz w-full p-4 rounded-xl text-lg pr-12"><button onclick="togglePw(\'login-password\')" class="absolute right-4 top-4 text-blue-300 hover:text-white">MATA</button></div><div class="text-right"><span onclick="navigateTo(\'forgot-password\')" class="text-blue-400 text-sm cursor-pointer hover:underline">Lupa kata sandi?</span></div><button onclick="handleLogin()" class="btn-diz w-full p-4 rounded-xl text-xl font-bold mt-4">MASUK</button><div class="text-center mt-4"><button onclick="loginWithGoogle()" class="w-full glass-card p-3 rounded-xl flex items-center justify-center gap-2 hover:bg-blue-600/30 transition text-sm"><span>G</span> Lanjutkan dengan Google</button></div><div class="text-center text-blue-300 mt-4">Belum punya akun? <span onclick="navigateTo(\'sign\')" class="text-blue-400 cursor-pointer hover:underline">Daftar</span></div></div></div>'
}
...
window.handleCompleteProfile = async function(userId) {
  ...
  currentUser = user
  setupHeader()
  showNotification('Profil berhasil dibuat. Link Anda: dizmark/' + username, 'success')
...
async function loadDashboardProducts() {
  ...
  grid.innerHTML = products.slice(0, 6).map(p => '<div class="glass-card p-4 rounded-xl cursor-pointer" onclick="navigateTo(\'product-detail\', { productId: \'' + p.id + '\' })"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/pe25yl.png') + '" alt="' + p.title + '" class="w-full h-40 object-cover rounded-lg mb-3" onerror="this.src=\'https://files.catbox.moe/pe25yl.png\'"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</p></div>').join('')
}
...
function displayMarket(products) {
  ...
  grid.innerHTML = products.map(p => '<div class="glass-card rounded-2xl overflow-hidden hover:scale-[1.02] transition cursor-pointer" onclick="navigateTo(\'product-detail\', { productId: \'' + p.id + '\' })"><div class="h-48 bg-diz-700"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/pe25yl.png') + '" alt="' + p.title + '" class="w-full h-full object-cover" onerror="this.src=\'https://files.catbox.moe/pe25yl.png\'"></div><div class="p-4"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300 truncate">' + (p.description || '').substring(0, 60) + '</p><div class="flex justify-between items-center mt-3"><span class="font-bold text-blue-400">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</span><span class="text-xs text-blue-400">' + (p.views_count || 0) + ' views</span></div></div></div>').join('')
}
...
async function loadFiles() {
  ...
  list.innerHTML = window.myProducts.map(p => '<div class="glass-card rounded-2xl overflow-hidden"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/pe25yl.png') + '" class="w-full h-48 object-cover" onerror="this.src=\'https://files.catbox.moe/pe25yl.png\'"><div class="p-4"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</p><div class="flex gap-2 mt-3"><button onclick="editProductForm(\'' + p.id + '\')" class="btn-diz flex-1 py-2 rounded-lg text-sm">EDIT</button><button onclick="deleteProductConfirm(\'' + p.id + '\')" class="glass-card flex-1 py-2 rounded-lg text-sm text-red-400">HAPUS</button></div></div></div>').join('')
}
...
function renderViewPage(container, username) {
  supabase.from('users').select('*').eq('username', username).single().then(({ data: user }) => {
    ...
    supabase.from('products').select('*').eq('user_id', user.id).eq('is_active', true).order('created_at', { ascending: false }).then(({ data: products }) => {
      container.innerHTML = '<div class="animate-slide-in max-w-4xl mx-auto"><div class="h-64 rounded-2xl overflow-hidden mb-4 bg-diz-700">' + (user.cover_url ? '<img src="' + user.cover_url + '" class="w-full h-full object-cover">' : '<div class="w-full h-full bg-gradient-to-r from-diz-600 to-diz-800"></div>') + '</div><div class="glass-card p-6 rounded-2xl -mt-20 relative z-10 mx-4 mb-8"><div class="flex flex-col md:flex-row items-center gap-6"><img src="' + (user.avatar_url || 'https://files.catbox.moe/pe25yl.png') + '" class="w-32 h-32 rounded-full object-cover border-4 border-diz-800 -mt-20"><div class="text-center md:text-left"><h1 class="text-3xl font-bold gradient-text">' + (user.full_name || user.username) + '</h1><p class="text-blue-300 text-lg">dizmark/' + user.username + '</p><p class="text-blue-200 mt-2">' + (user.bio || 'Tidak ada bio') + '</p></div></div>' + (user.phone ? '<div class="mt-4 text-center"><a href="https://wa.me/' + user.phone.replace(/\+/g, '') + '" target="_blank" class="btn-diz px-8 py-3 rounded-xl inline-block">WHATSAPP</a></div>' : '') + '</div><h3 class="text-2xl font-bold gradient-text mb-6">Produk</h3><div class="grid grid-cols-1 md:grid-cols-3 gap-6">' + (products && products.length > 0 ? products.map(p => '<div class="glass-card rounded-2xl overflow-hidden cursor-pointer" onclick="navigateTo(\'product-detail\', { productId: \'' + p.id + '\' })"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/pe25yl.png') + '" class="w-full h-48 object-cover" onerror="this.src=\'https://files.catbox.moe/pe25yl.png\'"><div class="p-4"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-blue-400 font-bold mt-2">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</p></div></div>').join('') : '<p class="text-blue-300 col-span-full text-center py-8">Belum ada produk</p>') + '</div></div>'
    })
  })
}
...
function renderAboutPage(container) {
  container.innerHTML = '<div class="max-w-2xl mx-auto py-12 text-center"><img src="https://files.catbox.moe/pe25yl.png" class="h-24 w-24 mx-auto rounded-full mb-6"><h2 class="text-3xl font-bold gradient-text mb-8">ABOUT US</h2><div class="glass-card p-6 rounded-2xl mb-6"><p class="text-blue-200">DIZMARK adalah platform jual beli produk digital.</p></div><div class="glass-card p-6 rounded-2xl"><h3 class="font-bold mb-4">Kirim Pesan</h3><textarea id="about-msg" rows="4" maxlength="500" placeholder="Tulis pesan (max 500 karakter)" class="input-diz w-full p-4 rounded-xl mb-3"></textarea><p class="text-xs text-blue-400 mb-3"><span id="about-count">0</span>/500</p><button onclick="sendAboutMsg()" class="btn-diz w-full py-3 rounded-xl font-bold">KIRIM</button></div></div>'
...
function renderContactPage(container) {
  container.innerHTML = '<div class="max-w-2xl mx-auto py-12 text-center"><img src="https://files.catbox.moe/pe25yl.png" class="h-24 w-24 mx-auto rounded-full mb-6"><h2 class="text-3xl font-bold gradient-text mb-8">CONTACT US</h2><div class="flex gap-8 mb-8"><a href="https://vm.tiktok.com/ZS98d811wU23L-6JMzD" target="_blank" class="glass-card flex-1 p-6 rounded-2xl hover:bg-blue-600/30"><p class="text-2xl font-bold">TIKTOK</p></a><a href="https://wa.me/6282122598130" target="_blank" class="glass-card flex-1 p-6 rounded-2xl hover:bg-blue-600/30"><p class="text-2xl font-bold">WHATSAPP</p></a></div><div class="glass-card p-6 rounded-2xl"><textarea id="contact-msg" rows="4" maxlength="500" placeholder="Tulis pesan" class="input-diz w-full p-4 rounded-xl mb-3"></textarea><button onclick="sendContactMsg()" class="btn-diz w-full py-3 rounded-xl font-bold">KIRIM</button></div></div>'
}
...
function renderTermsPage(container) {
  container.innerHTML = '<div class="max-w-3xl mx-auto py-12"><h2 class="text-3xl font-bold gradient-text mb-8">TERMS & CONDITIONS</h2><div class="glass-card p-6 rounded-2xl space-y-4 text-blue-200"><p>Dengan menggunakan DIZMARK, Anda setuju dengan syarat dan ketentuan ini.</p><p>Penjual bertanggung jawab atas produk yang dijual.</p><p>DIZMARK tidak memproses pembayaran. Transaksi langsung antara pembeli dan penjual.</p></div></div>'
}

function renderPrivacyPage(container) {
  container.innerHTML = '<div class="max-w-3xl mx-auto py-12"><h2 class="text-3xl font-bold gradient-text mb-8">PRIVACY</h2><div class="glass-card p-6 rounded-2xl text-blue-200"><p>DIZMARK menjaga privasi Anda. Data Anda tidak dibagikan ke pihak ketiga.</p></div></div>'
}

function renderPrivacyPolicyPage(container) {
  container.innerHTML = '<div class="max-w-3xl mx-auto py-12"><h2 class="text-3xl font-bold gradient-text mb-8">PRIVACY POLICY</h2><div class="glass-card p-6 rounded-2xl space-y-4 text-blue-200"><h3 class="text-xl font-bold text-white">1. Informasi Dikumpulkan</h3><p>Kami mengumpulkan informasi yang Anda berikan saat mendaftar: nama, email, telepon.</p><h3 class="text-xl font-bold text-white">2. Penggunaan</h3><p>Untuk menyediakan dan meningkatkan layanan DIZMARK.</p><h3 class="text-xl font-bold text-white">3. Keamanan</h3><p>Data disimpan dengan enkripsi.</p><h3 class="text-xl font-bold text-white">4. Hak Anda</h3><p>Anda dapat mengakses, mengoreksi, atau menghapus data Anda.</p></div></div>'
}
...
window.saveProfile = async function() {
  ...
  localStorage.setItem('dizmark_user', JSON.stringify(currentUser))
