...
async function loadDashboardProducts() {
  ...
  grid.innerHTML = products.slice(0, 6).map(p => '<div class="glass-card p-4 rounded-xl cursor-pointer" onclick="navigateTo(\'product-detail\', { productId: \'' + p.id + '\' })"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/jxo3mr.png') + '" alt="' + p.title + '" class="w-full h-40 object-cover rounded-lg mb-3" onerror="this.src=\'https://files.catbox.moe/jxo3mr.png\'"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</p></div>').join('')
}
...
function displayMarket(products) {
  ...
  grid.innerHTML = products.map(p => '<div class="glass-card rounded-2xl overflow-hidden hover:scale-[1.02] transition cursor-pointer" onclick="navigateTo(\'product-detail\', { productId: \'' + p.id + '\' })"><div class="h-48 bg-diz-700"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/jxo3mr.png') + '" alt="' + p.title + '" class="w-full h-full object-cover" onerror="this.src=\'https://files.catbox.moe/jxo3mr.png\'"></div><div class="p-4"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300 truncate">' + (p.description || '').substring(0, 60) + '</p><div class="flex justify-between items-center mt-3"><span class="font-bold text-blue-400">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</span><span class="text-xs text-blue-400">' + (p.views_count || 0) + ' views</span></div></div></div>').join('')
}
...
async function loadFiles() {
  ...
  list.innerHTML = window.myProducts.map(p => '<div class="glass-card rounded-2xl overflow-hidden"><img src="' + (p.thumbnail_url || 'https://files.catbox.moe/jxo3mr.png') + '" class="w-full h-48 object-cover" onerror="this.src=\'https://files.catbox.moe/jxo3mr.png\'"><div class="p-4"><h4 class="font-bold truncate">' + p.title + '</h4><p class="text-sm text-blue-300">' + (p.is_free ? 'FREE' : 'Rp' + (p.discount_price || p.price)?.toLocaleString()) + '</p><div class="flex gap-2 mt-3"><button onclick="editProductForm(\'' + p.id + '\')" class="btn-diz flex-1 py-2 rounded-lg text-sm">EDIT</button><button onclick="deleteProductConfirm(\'' + p.id + '\')" class="glass-card flex-1 py-2 rounded-lg text-sm text-red-400">HAPUS</button></div></div></div>').join('')
}
